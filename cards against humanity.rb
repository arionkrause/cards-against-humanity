...
def render_card_page(pdf, card_geometry, statements, is_black)
	...
	pdf.line_width(0.5);
		...
		#pdf.fill_and_stroke(:fill_color=>"000000", :stroke_color=>"000000") do
			#pdf.canvas do
				#pdf.rectangle([pdf.bounds.absolute_left + 10, pdf.bounds.absolute_top - 62], pdf.bounds.width - 20, pdf.bounds.height - 124)
			#end
		#end
	#end
	...
	statements.each_with_index do |line, idx|
		box(pdf, card_geometry, idx) do
			...
			# Top bar on black cards
			if is_black
				pdf.stroke_color "000000"
				pdf.fill_color "000000"

				#pdf.rotate(angle, :origin => [x, y]) do
				pdf.fill_and_stroke_rectangle([pdf.bounds.left - 10, pdf.bounds.top + 10], card_geometry["card_width"], 5)
				#end

				#pdf.rectangle([pdf.bounds.left + 25, pdf.bounds.top], card_geometry["card_width"], 5)
			...
			#pick 2
			if is_pick2
				pdf.text_box "JOGUE", size: 7, align: :right, width: 30, at: [pdf.bounds.right - 47, pdf.bounds.bottom + 19.5]
				pdf.fill_and_stroke(:fill_color => "ffffff", :stroke_color => "ffffff") do
					pdf.circle([pdf.bounds.right - 10, pdf.bounds.bottom + 17], 4)
				end

				pdf.fill_color "ffffff"
				pdf.text_box "2", size: 8, width: 8, align: :center, at:[pdf.bounds.right - 14, pdf.bounds.bottom + 20]
			end

			#pick 3
			if is_pick3
				pdf.text_box "PEGUE", size: 7, align: :right, width: 30, at: [pdf.bounds.right - 47, pdf.bounds.bottom + 29]
				pdf.fill_and_stroke(:fill_color => "ffffff", :stroke_color => "ffffff") do
					pdf.circle([pdf.bounds.right - 10, pdf.bounds.bottom + 26.5], 4)
				end

				pdf.fill_color "ffffff"
				pdf.text_box "2", size: 8, width: 8, align: :center, at:[pdf.bounds.right - 14, pdf.bounds.bottom + 29.5]
				pdf.fill_color "000000"
				pdf.text_box "JOGUE", size: 7, align: :right, width: 30, at: [pdf.bounds.right - 47, pdf.bounds.bottom + 18.5]

				pdf.fill_and_stroke(:fill_color => "ffffff", :stroke_color => "ffffff") do
					pdf.circle([pdf.bounds.right - 10, pdf.bounds.bottom + 16], 4)
				end

				pdf.fill_color "ffffff"
				pdf.text_box "3", size: 8, width: 8, align: :center, at:[pdf.bounds.right - 14, pdf.bounds.bottom + 19]
			end
		end
	end
end
...
def draw_logo(pdf, is_black)
	...
	pdf.text_box "Cards Against Humanity", size: 4.5, at: [pdf.bounds.left + 22, pdf.bounds.bottom + 18.5]
end
...
def load_pages_from_file(file, card_geometry)
	pages = []
	if File.exist?(file)
		lines = IO.readlines(file, :encoding => "UTF-8")
		pages = load_pages_from_lines(lines, card_geometry);
	end
	return pages
end
...
def render_cards(directory=".", white_file="white.txt", black_file="black.txt", output_file="cards.pdf", input_files_are_absolute=false, output_file_name_from_directory=true, recurse=true, card_geometry=get_card_geometry, white_string="", black_string="", output_to_stdout=false, title=nil )
	...
	if white_pages.length > 0 or black_pages.length > 0
		...
		white_pages.each_with_index do |statements, page|
			render_card_page(pdf, card_geometry, statements, false)
		...
		black_pages.each_with_index do |statements, page|
			render_card_page(pdf, card_geometry, statements, true)
	...
	if (not input_files_are_absolute) and recurse
		...
		files_in_dir.each do |subdir|
			if File.directory? subdir
				render_cards(subdir, original_white_file, original_black_file, "irrelevant", false, true, true, card_geometry )
			end
		end
	end
...
def print_help
	...
	puts "\tOR"
	puts "\tbbcards --white [WHITE_CARD_FILE] --black [BLACK_CARD_FILE] --output [OUTPUT_FILE]"
	puts ""
	...
	puts "black.txt. The output will be a pdf file with the same name as"
...
if not (ENV['REQUEST_URI']).nil?
	...
	render_cards nil, nil, nil, "cards.pdf", true, false, false, card_geometry, white_cards, black_cards, true
else
	arg_defs = Hash.new
	flag_defs = Hash.new
	arg_defs["-b"]		 = "black"
	...
	if args.has_key? "help" or args.length == 0 or ( (not args.has_key? "white") and (not args.has_key? "black") and (not args.has_key? "dir") )
		...
		render_cards args["dir"], "white.txt", "black.txt", "cards.pdf", false, true, true, card_geometry, "", "", false
	else
		render_cards nil, args["white"], args["black"], args["output"], true, false, false, card_geometry, "", "", false
	end
end
exit
